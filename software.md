---
layout: page
title: Software
---

A brief description of the universally useful software that I created during my graduate studies...  
**Software page is under construction.**


<!--
<a href="{{ 'git_stuff.html' | absolute_url }}" target="_blank" class="button">Git tutorial</a>
-->
<!--
 [many tools](http://rsl.stanford.edu/research/software.html)
-->

<!--
<a href="https://github.com/ekgibbons/epg-python" target="_blank" class="button">EPG python</a>
-->
